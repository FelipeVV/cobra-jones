...
int Game::run()
{
    ...
    this->view->setBackgroundBrush(QBrush(Qt::yellow, Qt::SolidPattern));
//	scene->addItem( new QGraphicsPixmapItem(QPixmap(":/Background.png")) );
    ...
    // create zooboo test object
    Zooboo* zooboo = new Zooboo();
    zooboo->setRect(0, 0, 50, 50);
    this->scene->addItem(zooboo);

    /*
    //focus on zooboo
    zooboo->setFlag(QGraphicsItem::ItemIsFocusable); //Make object the focus thus allowing it to be able to listen to keybard events
    zooboo->setFocus(); //tell object to recieve all keyboard events
    */
